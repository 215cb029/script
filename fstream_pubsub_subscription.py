...
def extract_job_name1(sub_name, topic_last_word, sub_last_word):
    """
    sub_name= projects/fkp-dart-pubsub/subscriptions/in-chennai-1.dart.fkpg.cp.payments.SaleTransaction--PanoramaSaleTransactionKafkaToKafka--sub
    topic_last_word= SaleTransaction
    sub_last_word= sub
    output_job_name= PanoramaSaleTransactionKafkaToKafka
    """
    try:
        start_index = sub_name.index(topic_last_word) + len(topic_last_word)
        end_index = sub_name.rindex(sub_last_word)
        between = sub_name[start_index:end_index]
        words = between.replace('/', ' ').replace('-', ' ').split()
        return ' '.join(words) if words else None
    except ValueError:
        return None

def extract_job_name2(subscription_path):
    """
    sub_name= projects/fkp-sonic-pubsub/subscriptions/PageViewJob-PageViewTask--retry-sub
    output_job_name= PageViewJob
...
def get_last_word(input_string):
    """
    get last word of both topic and sub
    """
...
def process_pubsub_file(sc):
    subset = data.get("pubsub_subscriptions", [])[0:1000] # in every time running increase the first index and last index
    rdd = sc.parallelize(subset, numSlices=10)

    # Map each subscription to a dictionary
    subscription_rows = rdd.map(lambda sub: {
        "Job Name": extract_job_name1(
            sub["name"],
            get_last_word(sub["topic_name"]),
            get_last_word(sub["name"])
        ) or extract_job_name2(sub["name"]),
        "Pubsub / Viesti / Kafka Topic Name": sub["topic_name"],
        "Pubsub/Viesti  Subscription Name": sub["name"],
        "Idle Subscription":precondition_check(sub["name"]),
        "ServiceAccounts": ", ".join(permissions_lookup.get(sub["name"], []))
    }).collect()
