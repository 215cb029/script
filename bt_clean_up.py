...
def process_rows(max_read_size,sc):
    global chunk_count
    args=get_args()
    chunk_size=args.batch_read_size
    table=get_table_object()
    partial_rows = list(table.read_rows(limit=max_read_size))
    last_row_key = None
    for chunk in get_chunks(partial_rows, chunk_size):
        ...
        rdd = sc.parallelize(chunk, numSlices=4)
        rdd.foreach(scan_rows)
...
def main():
 conf = SparkConf().setAppName("BigtableCleanup").setMaster("local[4]")
 ...
 try:
    ...
    if remaining_rows > 0:
        try:
            process_rows(remaining_rows,sc)
            time.sleep(bt_sleep_second)
        except Exception as e:
            logging.error(f"Error processing read_rows: {e}")
 finally:
